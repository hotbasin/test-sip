...
Для чистоты эксперимента предполагается организовать стенд, где проект работает
на хосте с &laquo;белым&raquo; IP-адресом (то есть напрямую смотрит в интернет
без NAT). Возможность есть, но нужно дополнительное время (отдельная тестовая
задачка :wink:).

Далее есть некоторые кандидаты на удачную отработку:
...
[**PJSUA2**](https://docs.pjsip.org/en/latest/pjsua2/intro.html). Хотя он предназначен в основном
для работы с Asterisk, но мало ли.
