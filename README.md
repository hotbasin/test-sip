...
6. Файл [**`client_confidentials_FAKE.py`**](client_confidentials_FAKE.py)
переименовать в **`client_confidentials.py`** (который есть в `.gitignore`,
чтобы не &laquo;светить&raquo; данные на GitHub) и в нём задать корректные
параметры для доступа на SIP-шлюз (IP-адрес или FQDN, логин и пароль).
7. Примерная последовательность команд в консоли терминала для запуска сервера
...
8. Для проверки работы зайти на корневую страницу-заглушку
`http://test_server:8080`. Там же есть гиперссылка на документацию Swagger
`http://test_server:8080/api/doc` (в случае запуска `main_swagger.py`).
9. Модуль тестирования `api_client.py` запускается вручную в той же виртуальной
